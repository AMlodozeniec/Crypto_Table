...
export default class Table extends Component {
	...
	flipDirection(column, headerTitle, currDirection) {
		...
		return column === headerTitle ? (currDirection === 'asc' ? 'desc' : 'asc') : 'desc'; //If different column is clicked, reset direction to descending. Otherwise, flip direction
	}

	handleSort = (headerTitle) => {
		let column = this.state.sortable.column;
		let currentDirection = this.state.sortable.direction;
		currentDirection = this.flipDirection(column, headerTitle, currentDirection);
